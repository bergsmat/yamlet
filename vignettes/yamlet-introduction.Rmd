...
Yamlet is a storage format for table metadata.  It was
designed to be:
...
Although intended mainly to document (or prespecify!) data column
...
the stored form must be valid yaml.

### Manual Method

Actually, `yamlet` (think: "just a little yaml")
is a special case of [yaml](https://yaml.org/spec/1.2/spec.html)
...
RACE: [ race, [ 0, 1, 2 ]]
```
So now we have codes for our dataset
that represent races.  What do these 
codes mean?  We supply decodes as keys.
...
## Attaching Yamlet To Tables

`meta` in the previous example 
is just a named list of column attributes.
...
has all the formatting options of ```read.table()``` and ```write.table()```).  The path is just the path
...
Obviously there's not much point in curating table metadata
unless you're going to do something interesting with it.
The earliest use of metadata is to guide table development
...
enrich output such as tables and figures consistently.  For example,
